...
interface ServicePageUIProps {
  ...
  serviceDict: ServiceDataDict;
}


export default function ServicePageUI({
  dict,
  locale,
  footerDict,
  serviceDict,
...
const sidebarLinks = Object.entries(serviceDict?.services || {}).map(([slug, data]) => (
  <li key={slug}>
    <ServiceLink
      serviceTitle={slug}
      locale={locale}
    ...
    >
      {slug === dict.slug ? dict.title : data.category}
    ...
    <div>
      ...
      <section className="min-h-[30vh] bg-[#330052] text-white font-sans relative">
        ...
        <div className="flex flex-col items-center justify-center text-center py-10 md:py-3 px-4">
          ...
          <Breadcrumb homeLabel={dict.breadcrumbHome} currentLabel={dict.title} locale={locale} />
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <aside className="w-full md:w-64 flex flex-col gap-6">
            <div className="bg-[#330052] rounded-t-xl p-6 text-white shadow-lg flex flex-col gap-6 relative pb-10 rounded-b-[40px]">
              <h3 className="text-lg font-semibold mb-2">{serviceDict.sidebar.allServices}</h3>
            ...
            <div className="bg-[#FFF9E5] text-[#330052] rounded-xl p-6 text-center font-medium shadow">
              ...
              {serviceDict.sidebar.help.title} <br />
              {serviceDict.sidebar.help.phone} <br />
              <strong>{serviceDict.sidebar.help.number}</strong>
            </div>
          </aside>

          {/* Section dynamique client */}
